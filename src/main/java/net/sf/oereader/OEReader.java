...
public class OEReader {
	...
	private ArrayList<OEMessageInfo> tree_Messages(OEData data, OETree t) {
		...
		ArrayList<OEMessageInfo> ret = new ArrayList<OEMessageInfo>(t.getBodyentries());
		for (int i = 0; i < t.getBodyentries(); i++)
			ret.add(new OEMessageInfo(data, t.value[i]));

		ret.addAll(tree_Messages(data, t.getDChild()));

		for (int i = 0; i < t.getBodyentries(); i++)
			ret.addAll(tree_Messages(data, t.getBChildren(i)));
